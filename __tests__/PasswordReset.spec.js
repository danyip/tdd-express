...
describe('Password Reset Request', () => {
  ...
  `(
    'it returns error body with $message for unknown email for password reset request when language is $language',
  ...
  it.each`
    ...
    ${'en'}  | ${en.email_invalid}
  `(
    'it returns 400 with validation error response having $message when request does not have valid email and language is $language',
    async ({ language, message }) => {
  ...
  `(
    'returns success response body with $message for known email for password reset request when langauge is set as $language',
    async ({ language, message }) => {
      const user = await addUser();
  ...
  it('creates passwordResetToken when a passowrd reset request is sent for a known email', async () => {
    const user = await addUser();
  ...
  it.each`
    ...
    ${'tr'}  | ${tr.email_failure}
    ${'en'}  | ${en.email_failure}
  `('returns $message when langauge is set as $language after email failure', async ({ language, message }) => {
    simulateSmtpFailure = true;
    const user = await addUser();
    const response = await postPasswordReset(user.email, { language });
    expect(response.body.message).toBe(message);
...
describe('Password Update', () => {
  ...
  `(
    'it returns error body with $message when language is set to $language after trying to update with invalid token',
    async ({ language, message }) => {
      const nowInMillis = new Date().getTime();
      const response = await putPasswordUpdate(
        {
  ...
  it.each`
    ...
    ${'en'}  | ${'lower4nd56789'} | ${en.password_pattern}
    ${'en'}  | ${'UPPER4ND5678'}  | ${en.password_pattern}
    ...
    ${'tr'}  | ${'lower4nd56789'} | ${tr.password_pattern}
    ${'tr'}  | ${'UPPER4ND5678'}  | ${tr.password_pattern}
  `(
    'returns password validation error $messafe when language is set to $language and value is $value',

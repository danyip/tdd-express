...
describe('Post Hoax', () => {
  ...
  `(
    'returns 400 and $message when hoax content is less than 10 chars language is $language',
    async ({ language, message }) => {
      await addUser();
  ...
  it('associates keeps the old association is a new hoax is submitted with old attachment id', async () => {
    const uploadResponse = await uploadFile();
    const uploadedFileId = uploadResponse.body.id;
    await addUser();
    await postHoax(

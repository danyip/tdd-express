...
describe('Upload file for hoax', () => {
  it('returns 200 ok after sucessful upload', async () => {
  ...
  it.each`
    ...
    ${'test-txt.txt'} | ${null}
  `('saves filetype in $fileType in attachment object when $file is uploaded', async ({ fileType, file }) => {
  ...
  it.each`
    ...
    ${'test-txt.txt'} | ${null}
  `(
    'saves filename with extension of $fileExtension in attachment object and stored object when $file is uploaded',
  ...
  it('returns 400 when uploaded file size is bigger then 5mb', async () => {
  ...
  `('returns $message when attachment size is larger then 5MB', async ({ language, message }) => {
    const fiveMB = 5 * 1024 * 1024;
    const filePath = path.join('.', '__tests__', 'resources', 'random-file');
    fs.writeFileSync(filePath, 'a'.repeat(fiveMB) + 'a');
